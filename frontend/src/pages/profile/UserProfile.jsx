...
export default function UserProfile() {
  let getasccessToken = useContext(RefreshTokenCotext);
  const [user, setUser] = useState();
  const [failed, setError] = useState(false);

  useEffect(() => {
    if (getasccessToken.RefreshToken) {
      const accessToken = getasccessToken.RefreshToken;
      axios
        .get(get_user, {
          headers: {
        ...
        <>
          ...
          <div className="bg-gray-100 h-screen flex items-center justify-center p-2">
            <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
              ...
              {/* Bio */}
              <p className="text-gray-700 mt-2">{user.data.fullName}</p>
              {/* Social Links */}
              <div className="flex items-center mt-4 space-x-4 justify-between">
                <p className="text-blue-500 hover:underline">
                  {user.data.email}
                </p>
                <p className="text-slate-500 hover:underline m-10">
                  since:- {user.data.createdAt.slice(0, 10)}
                </p>
              </div>
              {/* Separator Line */}
